...
#' @keywords player_rating_history_all
#' @export
#' @examples
...
player_rating_history_all <- function(player_id, game="aoe2de", id_to_text = TRUE) {

  start=0
  count=1000

  data <- player_rating_history(player_id = player_id, start=0, count = count)

  data_part <- data

  while (nrow(data_part) == count) {
    data_part <-  player_rating_history(player_id = player_id, start = start, count = count)
