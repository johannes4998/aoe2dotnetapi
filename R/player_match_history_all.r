...
#' @keywords player_match_history_all
#' @export
#' @examples
#' #Rating Match History of GL. TheViper with profile_id
#' player_match_history_all(player_id=196240)
#' #Rating Match History of GL. TheViper with steam_id
#' player_match_history_all(player_id="76561197984749679")


player_match_history_all <- function(player_id, game="aoe2de", id_to_text = TRUE) {

  start=0
  count=1000

  data <- player_match_history(player_id = player_id, start=0, count = count)

  data_part <- data

  while (nrow(data_part) == count) {
    data_part <-  player_match_history(player_id = player_id, start = start, count = count)
